...
func (u *strategy) AveragePrice(in *internaldto.InputStrategyAveragePriceDto) (out []*internaldto.OutputStrategyAveragePriceDto, err error) {
	...
	if err != nil {
		log.Panicln("AP 01: ", err)
	}
	fmt.Println(operations)

	avg, err := usecase.NewUsecaseGetAvgPriceByParityExchange(u.AverageRepo).GetAvgPriceByParityExchange(&usecasedto.InputGetAvgPriceByParityExchangeDto{
		Parity:   in.TradeConfig.Parity,
		Exchange: in.TradeConfig.Exchange,
	})
	if err != nil {
		log.Panicln("AP 02: ", err)
	}
	fmt.Println(avg)
	...
	for _, operation := range operations {
		...
		if wallet.Amount > (in.TradeConfig.OperationAmount+in.TradeConfig.WalletValueLimit) && lastRegister.CoinQuantity == 0 {
			if currentPrice > avg.Day {
				fmt.Println(wallet.Amount, in.TradeConfig.OperationAmount, " + ", in.TradeConfig.WalletValueLimit)
				continue
			}
